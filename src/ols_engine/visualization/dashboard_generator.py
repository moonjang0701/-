...
Streamlit 기반의 대화형 웹 애플리케이션을 생성합니다.
"""
...
class DashboardGenerator:
    ...
    def __init__(self):
        ...
        self.chart_viz = ChartVisualizer()
    
    def create_streamlit_app(self, app_file: str) -> str:
        """
        Streamlit 애플리케이션 파일 생성
        
        Args:
            app_file: 생성할 Streamlit 앱 파일 경로
...
'''
        ...
        # 파일 저장
        output_path = Path(app_file)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(app_content)
        
        logger.info(f"Streamlit 앱 생성 완료: {app_file}")
        return str(output_path)
    
    def generate_comprehensive_visualization(self, penetration_gdf: gpd.GeoDataFrame,
                                          summary: Dict, results: List,
                                          runway_configs: List[dict],
                                          output_dir: str,
                                          ols_grid_gdf: Optional[gpd.GeoDataFrame] = None) -> Dict[str, str]:
        """
        종합 시각화 파일들 생성
        ...
        try:
            # 1. 대화형 지도
            map_file = output_path / "penetration_map.html"
            generated_files['map'] = self.map_viz.create_penetration_map(
                penetration_gdf, runway_configs, str(map_file),
                include_grid=ols_grid_gdf is not None, ols_grid_gdf=ols_grid_gdf
            )
            
            # 2. 대시보드 차트
            dashboard_file = output_path / "dashboard.html"
            generated_files['dashboard'] = self.chart_viz.create_penetration_dashboard(
                results, summary, str(dashboard_file)
            )
            
            # 3. 3D 시각화
            viz_3d_file = output_path / "3d_visualization.html"
            generated_files['3d'] = self.chart_viz.create_3d_visualization(
                penetration_gdf, ols_grid_gdf, runway_configs, str(viz_3d_file)
            )
            
            # 4. 요약 보고서 차트
            summary_chart_file = output_path / "summary_report.png"
            generated_files['summary_chart'] = self.chart_viz.create_summary_report(
                summary, str(summary_chart_file)
            )
            
            # 5. 활주로 단면도 (첫 번째 활주로)
            if runway_configs:
                try:
                    from ..core.geometry import OLSGeometry
                    
                    first_runway = runway_configs[0]
                    ols_geometry = OLSGeometry(first_runway, summary.get('arp_elev_m', 0.0))
                    
                    profile_file = output_path / f"profile_{first_runway['rwy_id'].replace('-', '_')}.png"
                    generated_files['profile'] = self.chart_viz.create_runway_profile(
                        penetration_gdf, first_runway, ols_geometry, str(profile_file)
                    )
                except ImportError as e:
                    logger.warning(f"OLSGeometry 임포트 실패, 단면도 생성 건너뜀: {str(e)}")
                    generated_files['profile'] = None
            
            # 6. Streamlit 앱
            streamlit_file = output_path / "streamlit_app.py"
            generated_files['streamlit'] = self.create_streamlit_app(str(streamlit_file))
            
            # 7. 통합 HTML 인덱스 페이지
            index_file = output_path / "index.html"
            generated_files['index'] = self._create_index_html(generated_files, str(index_file))
            
            logger.info(f"종합 시각화 생성 완료: {len(generated_files)}개 파일")
    ...
    def _create_index_html(self, file_paths: Dict[str, str], output_file: str) -> str:
        """통합 인덱스 HTML 생성"""
...
<body>
    <div class="hero">
        <div class="container">
            <h1 class="display-4">✈️ OLS 침투 분석 결과</h1>
            <p class="lead">장애물제한표면 침투 분석 시각화 대시보드</p>
        </div>
    </div>
        ...
        <div class="row g-4">
            <div class="col-md-6 col-lg-4">
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <h5 class="card-title">대화형 지도</h5>
                        <p class="card-text">침투 장애물과 OLS 표면을 지도에서 확인하세요.</p>
                        <a href="{Path(file_paths.get('map', '')).name}" class="btn btn-primary">지도 보기</a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <p class="card-text">다양한 차트와 통계를 한눈에 확인하세요.</p>
                        <a href="{Path(file_paths.get('dashboard', '')).name}" class="btn btn-success">대시보드</a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card">
                    <div class="card-body text-center">
                        <div class="icon">🎮</div>
                        <h5 class="card-title">3D 시각화</h5>
                        <p class="card-text">입체적인 3D 뷰로 침투 현황을 확인하세요.</p>
                        <a href="{Path(file_paths.get('3d', '')).name}" class="btn btn-info">3D 보기</a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card">
                    <div class="card-body text-center">
                        <div class="icon">📈</div>
                        <h5 class="card-title">요약 보고서</h5>
                        <p class="card-text">침투 분석 결과 요약 차트를 확인하세요.</p>
                        <a href="{Path(file_paths.get('summary_chart', '')).name}" class="btn btn-warning">보고서</a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card">
                    <div class="card-body text-center">
                        <div class="icon">📏</div>
                        <h5 class="card-title">활주로 단면도</h5>
                        <p class="card-text">활주로 중심선을 따른 단면도를 확인하세요.</p>
                        <a href="{Path(file_paths.get('profile', '')).name}" class="btn btn-secondary">단면도</a>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card">
                    <div class="card-body text-center">
                        <div class="icon">⚡</div>
                        <h5 class="card-title">대화형 앱</h5>
                        <p class="card-text">Streamlit 기반 실시간 분석 도구입니다.</p>
                        <div class="alert alert-info mt-2" role="alert">
                            <small>터미널에서 실행: <code>streamlit run {Path(file_paths.get('streamlit', '')).name}</code></small>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row mt-5">
            <div class="col">
                <div class="alert alert-light" role="alert">
                    <h4 class="alert-heading">📋 사용 안내</h4>
                    <ul>
                        <li><strong>지도</strong>: 클릭하여 장애물 상세 정보 확인</li>
                        <li><strong>대시보드</strong>: 대화형 차트로 데이터 탐색</li>
                        <li><strong>3D 시각화</strong>: 마우스로 회전/확대/축소 가능</li>
                        <li><strong>Streamlit 앱</strong>: 실시간 파일 업로드 및 분석</li>
                    </ul>
                </div>
            </div>
        </div>
